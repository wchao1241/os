...
func engineCreate(c *cli.Context) error {
	...
	fixIP := c.String("fix-ip")

	// generate & create engine compose
	err := generateEngineCompose(name, version, sshPort, authorizedKeys, network, fixIP)
	if err != nil {
		return err
	}
...
func preFlightValidate(c *cli.Context) error {
	...
	err = CheckUserDefineIPv4Address(fixIP, *userDefineNetwork)
	if err != nil {
		return err
	}

	isVersionMatch := false
	for _, v := range MultiEngineVersions {
		if v == version {
			isVersionMatch = true
			break
		}
	...
	return errors.Errorf("PreFlight check failed on engine create")
}
...
func generateEngineCompose(name, version string, sshPort int, authorizedKeys, network, fixIP string) error {
	...
	composeConfigs[name] = composeConfig.ServiceConfigV1{
		...
		Labels: composeYaml.SliceorMap{
			...
			config.UserDockerFIPLabel: fixIP,
		},
	}
